    ...
    datatable = $('#BooksTable').DataTable({
        ...
        columns: [
                    ...
                    return `<div class="symbol symbol-50px overflow-hidden me-3">
                                                <a href="/Books/Details/${row.id}">
                                                    <div class="symbol-label h-70px">
                                                        <img src="${(row.imageUrlThumbnail === null ? '/images/books/no-book.jpg' : row.imageUrlThumbnail)}" alt="cover" class="w-100">
                                                    </div>
                                                </a>
                                            </div>
            ...
            },
            {
                "name": "IsDeleted",
                "render": function (data, type, row) {
                    return `<span class="badge badge-light-${(row.deleted ? 'danger' : 'success')} js-status">
                                                ${(row.deleted ? 'Deleted' : 'Available')}
                                            </span>`;
                }
            },
            {
                ...
                "render": function (data, type, row) {
                    return `<a href="#" class="btn btn-light btn-active-light-primary btn-sm" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
                            ...
                            <!--begin::Menu item-->
                            <div class="menu-item px-3">
                                        <a href="javascript:;" class="menu-link flex-stack px-3 js-changstatus" data-url="/Books/ToggleStatus/${row.id}">
                                    Toggle Status
                                </a>
                            </div>
                            <!--end::Menu item-->
